...
below 0.05. Every levi test controls its search with a maximum statistic, so
this fraction should be at most 0.05.
...
out twenty times with Fruchterman-Reingold, and the node-label regional test
...
The `airway` experiment (Himes et al. 2014) measured RNA-seq in four airway
smooth muscle cell lines, each treated with dexamethasone and left untreated.
DESeq2 with `~ cell + dex` was run once, offline; the 80 genes with the
strongest response that STRING recognised, their STRING interactions
...
The rewiring null keeps every gene's Wald statistic and every node's degree
and asks whether the clusters of strong responders need the specific STRING
