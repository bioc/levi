...
**levi** provides four adapter functions that convert DE tool outputs directly
into the data.frame format expected by `expressionInput`.
...
[STRING database](https://string-db.org) and computes a 2-D layout
automatically. No manual file download is required.
...
| `"kk"` | Kamada-Kawai | Small networks (≤ 100 nodes) |
...
treated with dexamethasone (DEX) vs untreated controls (4 cell lines, ~64k
genes). We use DESeq2 to identify DE genes and `leviFromSTRING()` to build
...
The `ALL` dataset (Affymetrix HG-U95Av2, 128 patients) compares B-cell and
...
GUI-only features:
- **Brush selection** — click and drag on the landscape to compute scores for
  a custom region; gene list is shown below the plot.
- **Gene search** — type a gene symbol to highlight it with a yellow circle.
- **Peak labels** — checkbox overlays gene names at detected peak/valley
  positions (uses `ggrepel` if installed).
- **Progress bar** — shown during permutation test with per-iteration feedback.
- **Download buttons** — plot (PNG/PDF), score table (CSV), peak table (CSV).
