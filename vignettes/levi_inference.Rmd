...
regions without a further adjustment.
...
all. `leviGraphTFCEInference()` fits a limma moderated t per gene and
integrates it over thresholds with threshold-free cluster enhancement, using
network components as clusters:
...
the residuals of the nuisance-only model instead of the labels. The
single-cell functions aggregate cells into donor pseudobulks and permute the
donors, jointly across cell types, so that cells are never treated as
replicates.
...
same degree sequence. They ask whether the observed clustering needs the
...
spatial statistics used in geography. Both use the node-label null. The local
tables carry a Benjamini-Hochberg adjustment across nodes, and the Getis-Ord
